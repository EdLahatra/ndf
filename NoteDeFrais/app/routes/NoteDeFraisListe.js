...
export default class NoteDeFraisListe extends ListEdit {
  ...
  _renderRow(element) {
    ...
    return (
      <TouchableOpacity
        onPress={this.onPress.bind(this, element, isSelected)}
  ...
  _renderRowContent(element) {
    ...
    return (<View style={[{ flex: 1, flexDirection: 'row' }]}>
      <View style={{ flex: 4 }}>
        ...
        <Text style={[Style.h4, Colors.greyDarker.color()]}>
          {I18n.t('NoteDeFrais.period',
            { first: this._formatDate(firstDate), last: this._formatDate(lastDate) })}
        </Text>
      </View >
      <Text style={[Style.h3]}>
        {Math.round((element.totalDepenses + element.totalIndemnitesKilometriques) * 100) / 100}
        {I18n.t('devise')}
      </Text>
    </View>);
  }
