...
export default class GooglePlacesAutocomplete extends Component {
  ...
  getInitialState = () => ({
    ...
    listViewDisplayed: this.props.listViewDisplayed === 'auto'
      ? false : this.props.listViewDisplayed,
  })
  ...
  _requestNearby = (latitude, longitude) => {
    ...
    if (
      latitude !== undefined && longitude
      !== undefined && latitude !== null && longitude !== null
    ) {
      const request = new XMLHttpRequest();
      this._requests.push(request);
      request.timeout = this.props.timeout;
  ...
  _getFlatList = () => {
    ...
    if (
      (this.state.text !== '' || this.props.predefinedPlaces.length
